...
int resolve_probe_addresses(struct net_state_t* net_state,
    ...
    if (decode_address_string(param->ip_version, param->remote_address,
                              dest_sockaddr)) {
    ...
    if (param->local_address) {
        if (decode_address_string(param->ip_version, param->local_address,
                                  src_sockaddr)) {
    ...
    if (param->protocol == IPPROTO_ICMP) {
        if ((src_sockaddr->ss_family == AF_INET &&
             !net_state->platform.ip4_socket_raw) ||
            (src_sockaddr->ss_family == AF_INET6 &&
             !net_state->platform.ip6_socket_raw))
...
*/
struct probe_t* find_probe(struct net_state_t* net_state, int protocol, int id,
                           int sequence) {
    struct probe_t* probe;
...
*/
static void format_mpls_string(char* str, int buffer_size, int mpls_count,
    ...
    for (i = 0; i < mpls_count; i++) {
        ...
        snprintf(append_pos, buffer_size, "%d,%d,%d,%d", mpls->label,
                 mpls->traffic_class, mpls->bottom_of_stack, mpls->ttl);
...
void respond_to_probe(struct net_state_t* net_state, struct probe_t* probe,
    ...
    if (inet_ntop(remote_addr->ss_family, sockaddr_addr_offset(remote_addr),
                  ip_text, INET6_ADDRSTRLEN) == NULL) {
        error(EXIT_FAILURE, errno, "inet_ntop failure");
    }

    snprintf(response, COMMAND_BUFFER_SIZE, "%d %s %s %s round-trip-time %d",
